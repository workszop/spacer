...
const drawProp=html.slice(html.indexOf('function drawProp('),html.indexOf('function drawRug('));
const flat=html.match(/const FLAT_PROPS=new Set\(\[([^\]]*)\]\)/)[1].match(/[a-z]+/g);
const handled=new Set([...[...(drawBlock+drawProp).matchAll(/kind==='([a-z]+)'/g)].map(m=>m[1]),...flat]);
assert.match(html,/function drawTape\(/,'flat hazard tape has a floor-pass renderer');
